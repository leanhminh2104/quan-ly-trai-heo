...
import { Edit, Trash2, Search, Plus, Activity } from 'lucide-react'
...
export default function BoarsPage() {
  ...
  const columns: ColumnDef<PigWithDetails>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-semibold text-indigo-700 dark:text-indigo-400">{row.original.earTag || row.original.code}</span>
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-8 w-8 text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/50" 
            title="Lịch sử khai thác tinh"
            onClick={() => alert('Tính năng Khai thác tinh đang được xây dựng')}
          >
          ...
          <Button 
            ...
            onClick={() => {
              if (confirm(`Bạn có chắc chắn muốn xóa đực giống mã ${row.original.code}?`)) {
  ...
  return (
    <div className="space-y-6 p-4 md:p-8 pt-6">
      <PageHeader
        title="Quản lý đực giống"
        description="Theo dõi danh sách và tình trạng khai thác của đàn đực giống"
        action={{
          label: 'Thêm đực giống',
          href: '/pigs/new?type=BOAR',
      ...
      <DataTable
        ...
        emptyTitle="Không tìm thấy đực giống"
        emptyDescription="Chưa có dữ liệu đực giống hoặc không khớp với bộ lọc."
      />
    </div>
  )
}
