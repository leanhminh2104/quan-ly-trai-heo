...
export default function GiltsPage() {
  ...
  const columns: ColumnDef<PigWithDetails>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-semibold text-lime-700 dark:text-lime-400">{row.original.earTag || row.original.code}</span>
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-8 w-8 text-lime-600 hover:text-lime-700 hover:bg-lime-50 dark:hover:bg-lime-950/50" 
            title="Chuyển lên nái"
            onClick={() => {
              if (confirm(`Chuyển hậu bị ${row.original.code} lên nái?`)) {
                updatePig({ id: row.original.id, data: { type: 'SOW' } })
              }
            }}
            disabled={isUpdating}
          >
          ...
          <Button 
            ...
            onClick={() => {
              if (confirm(`Bạn có chắc chắn muốn xóa hậu bị mã ${row.original.code}?`)) {
  ...
  return (
    <div className="space-y-6 p-4 md:p-8 pt-6">
      <PageHeader
        title="Quản lý lợn hậu bị"
        description="Theo dõi đàn lợn hậu bị và đánh giá tiêu chuẩn lên nái"
        action={{
          label: 'Thêm lợn hậu bị',
          href: '/pigs/new?type=GILT',
      ...
      <DataTable
        ...
        emptyTitle="Không tìm thấy lợn hậu bị"
        emptyDescription="Chưa có dữ liệu lợn hậu bị hoặc không khớp với bộ lọc."
      />
    </div>
  )
}
