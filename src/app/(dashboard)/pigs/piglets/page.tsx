...
import { Edit, Trash2, Search, Plus, UserMinus } from 'lucide-react'
...
export default function PigletsPage() {
  ...
  const columns: ColumnDef<PigWithDetails>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-semibold text-cyan-700 dark:text-cyan-400">{row.original.earTag || row.original.code}</span>
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <Button 
            variant="ghost" 
            size="icon" 
            className="h-8 w-8 text-cyan-600 hover:text-cyan-700 hover:bg-cyan-50 dark:hover:bg-cyan-950/50" 
            title="Cai sữa"
            onClick={() => alert('Tính năng Cai sữa đang được xây dựng')}
          >
  ...
  return (
    <div className="space-y-6 p-4 md:p-8 pt-6">
      <PageHeader
        title="Quản lý lợn con"
        description="Theo dõi đàn lợn con theo mẹ và lịch cai sữa"
        action={{
          label: 'Nhập đàn',
          href: '/pigs/new?type=PIGLET',
        ...
        <div className="flex gap-4">
          <Select value={status || 'all'} onValueChange={(val) => setStatus(val === 'all' ? undefined : val as PigStatus)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="ACTIVE">Đang theo mẹ</SelectItem>
      ...
      <DataTable
        ...
        emptyTitle="Không tìm thấy lợn con"
        emptyDescription="Chưa có dữ liệu lợn con hoặc không khớp với bộ lọc."
      />
    </div>
  )
}
